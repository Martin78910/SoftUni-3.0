...
public class Problem26 {
    public static void main(String[] args) {
        ...
        Integer[] firstCircleInfo = Arrays.stream(scan.nextLine().split("\\s+")).map(Integer::parseInt).toArray(Integer[]::new);

        Circle firstCircle = new Circle(firstCircleInfo[0], firstCircleInfo[1], firstCircleInfo[2]);

        Integer[] secondCircleInfo = Arrays.stream(scan.nextLine().split("\\s+")).map(Integer::parseInt).toArray(Integer[]::new);
